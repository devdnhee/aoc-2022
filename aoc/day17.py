...
class Chamber:
    ...
    @property
    def floor(self):
        return int(min(map(lambda pos: pos.imag, self.environment[-Chamber.TAIL_CHECK:])))
    ...
    def df_simulation(self):
        df = (pd.DataFrame(self.history, columns=['number_moves', 'n_rocks', 'height'])
              .assign(height_rock_ratio = lambda df: df.height / df.n_rocks, 
    ...
    def check_move(self, move):
        ...
        if moved_rock[-1].real >= self.width or moved_rock[0].real < 0:
            return False
        if any(map(lambda pos: pos in self.environment[-Chamber.TAIL_CHECK:], moved_rock)):
            return False
        return True
    ...
    def find_pattern_period(self, pattern_length):
        ...
        matches = np.atleast_1d(np.squeeze(np.argwhere(list(map(lambda a: np.all(pattern == a), split_sequence)))))
    ...
    def simulation(self, moves, N=2022, find_repeats=False, pattern_length=2000):
        ...
        for m_idx, move_enc in enumerate(moves_generator):
            ...
            if self.falling_rock is None:
                ...
                if find_repeats and cycle_period is None and n_rocks > 1 and n_rocks % pattern_divisor == pattern_modulo:
                    print(f"Checking for pattern after {n_rocks} rocks: pattern_length={pattern_length}")
                    ...
                    if cycle_period is not None:
                        print(f"Detected pattern after {n_rocks} rocks: cycle_period={cycle_period}")
                
                if cycle_period is not None and (n_rocks % cycle_period == N % cycle_period):
                    height_per_cycle = self.height - self.df_simulation.iloc[-cycle_period-1].height
            ...
            if self.check_move(Chamber.MOVES['v']):
                ...
                self.height = int(max(map(lambda pos: pos.imag, self.environment[-Chamber.TAIL_CHECK:])))
                self.falling_rock = None
