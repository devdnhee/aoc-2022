...
class Cave:
    ...
    def __str__(self):
        first_column_show = np.argwhere(self.environment != 0)[:, 1].min() - 1 # -1 to show a bit of left side
        last_column_show = np.argwhere(self.environment != 0)[:, 1].max() + 1  # +1 to show a bit of right side
    ...
    def draw_line(cls, environment, from_coord, to_coord, value=ROCK):
        if from_coord[0] == to_coord[0]:
            ...
            r = from_coord[0]
            c_from, c_to = (from_coord[1], to_coord[1]) if from_coord[1] < to_coord[1] else (to_coord[1], from_coord[1])
            environment[r, c_from:c_to+1] = value
        elif from_coord[1] == to_coord[1]:
            # draw vertical line
            c = from_coord[1]
            r_from, r_to = (from_coord[0], to_coord[0]) if from_coord[0] < to_coord[0] else (to_coord[0], from_coord[0])
    ...
    def from_string(cls, string, sand_leak=(0, 500), max_dimension=1000, floor=False):
        rocks = [
            [tuple(map(int, reversed(match.split(',')))) for match in re.findall("\d+,\d+", line)]
        ...
        n_rows = max_row + 3 # +1 for 0, +1 for under the deepest rock and detect abyss, +1 for floor
