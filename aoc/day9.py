...
class Rope():
    ...
    def __str__(self):
        min_x, max_x = min(int(min([r.real for r in self.rope])), 0), max(int(max([r.real for r in self.rope])), 0)
        min_y, max_y = min(int(min([r.imag for r in self.rope])), 0), max(int(max([r.imag for r in self.rope])), 0)
